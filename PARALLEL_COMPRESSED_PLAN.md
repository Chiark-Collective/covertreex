...
- 2025-11-05 (late night, adjacency scatter pass) — Reworked `_scope_numba.py` so only deduped scope groups participate in pair expansion (no per-node zero work) and the directed edge emission runs with `nb.prange`. Candidate capacity is reserved per surviving group, and a compact pass trims unused slots after radius pruning.
  - Quick benchmark (2 048×512×k=8, diagnostics off) now reports dominated-batch `conflict_adj_scatter_ms ≈ 0.32–0.40` after the initial compilation batch (first dominated batch still pays ~3 s for the new kernel’s JIT, but subsequent batches stay <0.4 ms). Steady-state build drops to **~0.21 s/batch** and query remains **6 ms**, matching the earlier traversal speedups.
  - 8 k benchmark (`--tree-points 8192 --batch-size 256 --queries 1024 --k 16`, diagnostics off) finishes in **3.11 s build / 0.052 s query** (build CPU 5.19 s, util 1.66×; query CPU 1.14 s, util 22.1×). Dominated batches shrink to `conflict_adj_scatter_ms ≈ 0.85–1.16 ms` once the kernel is hot, though the first dominated batch still spends ~14–17 ms inside the scatter loop. Memory deltas grow to ≈230 MB due to the larger scope arrays; future work should recycle the directed buffers to avoid repeated allocations.
  - Follow-up: explore pre-JIT warming (e.g., zero-length calibration batch) so the 3 s compilation spike cannot hit production builds, and profile the remaining 14–17 ms scatter time on the first dominated batch—likely reducible by caching the per-group norms used in the radius checks.
