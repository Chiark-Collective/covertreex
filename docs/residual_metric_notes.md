...
_Status snapshot (2025-11-17)._ Dense Hilbert 32 k residual builds now complete in **≈20–21 s** wall time when the bitset scope streamer is enabled by default (`pcct-20251114-141549-e500d8`, log `artifacts/benchmarks/artifacts/benchmarks/residual_dense_32768_dense_streamer_bitset.jsonl`). Dominated batches report `traversal_semisort_ms≈47 ms` (p90 ≈66 ms) thanks to the dense streamer, masked append, and the new Numba-backed bitset dedupe. The 4 k guardrail (`pcct-20251114-105559-b7f965`, log `artifacts/benchmarks/artifacts/benchmarks/residual_phase05_hilbert_4k_dense_streamer_gold.jsonl`) stays well below the `<1 s` requirement (`traversal_semisort_ms≈42 ms`). Use `--no-residual-dense-scope-streamer`, `--no-residual-scope-bitset`, or `--no-residual-masked-scope-append` only when you need to reproduce the historical multi-pass telemetry.
