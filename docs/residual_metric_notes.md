...
**Latest reruns (2025-11-15).** 
- `pcct query` replayed the dense-streamer preset using the Typer surface (`--metric residual --tree-points 32768 --batch-size 512 --queries 1024 --k 8 --batch-order hilbert --disable-sparse-traversal --scope-chunk-target 0 --residual-dense-scope-streamer --residual-scope-bitset --residual-level-cache-batching`). Wall-clock via `time` landed at **27.439 s** (`pcct | build=25.47 s`, throughput ≈33.3 k q/s). Artifacts: log `artifacts/benchmarks/residual_dense_32768_dense_streamer_pairmerge_gold_rerun2.jsonl`, stdout `artifacts/benchmarks/residual_dense_32768_dense_streamer_pairmerge_gold_rerun2.stdout`. `run_id=pcct-20251115-195422-e03d5a`, `run_hash=3bcbd9d23e1bcd242e19f76db4716494256bf42a012d647baf6eaebe1959622a`.
- The historical `cli.queries` command (exact flags in the section below) was also rerun with the original environment toggles (`COVERTREEX_ENABLE_SPARSE_TRAVERSAL=1`, dense conflict builder, pair-merge/buffer reuse). That invocation measured **25.877 s** wall (`pcct | build≈25.2 s`, throughput ≈34.5 k q/s). Artifacts: log `artifacts/benchmarks/residual_dense_32768_dense_streamer_pairmerge_gold_cli_rerun3.jsonl`, stdout `artifacts/benchmarks/residual_dense_32768_dense_streamer_pairmerge_gold_cli_rerun3.stdout`. `run_id=pcct-20251115-195931-184755`, `run_hash=b39d86ceff4abac17b7284fe15117cb0032d6e507e023bb2b749282d13efae96`. Use these logs when diffing against the November 14 gold artefact; the command block below remains the authoritative reproduction recipe.
