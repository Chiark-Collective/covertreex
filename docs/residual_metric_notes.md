...
_Status snapshot (2025-11-17)._ Dense Hilbert 32 k residual builds now complete in **≈18.4 s** wall time / **16.2 s** dominated traversal with the dense streamer + level-cache batching preset (`pcct-20251114-162220-9efaf0`, log `artifacts/benchmarks/residual_dense_32768_dense_streamer_levelcache_rerun.jsonl`, `median traversal_semisort_ms≈37 ms`). Running the same command with diagnostics disabled yields **19.09 s** best-of-three wall time. The 4 k guardrail (`pcct-20251114-162122-761439`, log `artifacts/benchmarks/artifacts/benchmarks/artifacts/benchmarks/residual_phase05_hilbert_4k_dense_streamer_levelcache.jsonl`) stays well below the `<1 s` requirement (`traversal_semisort_ms≈19.8 ms`). Use `--no-residual-dense-scope-streamer`, `--no-residual-scope-bitset`, `--no-residual-masked-scope-append`, or `--no-residual-level-cache-batching` only when you need to reproduce historical telemetry.
